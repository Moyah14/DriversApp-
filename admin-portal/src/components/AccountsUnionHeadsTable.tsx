...
import UnionHeadDetailsDrawer from "./UnionHeadDetailsDrawer";
...
function mapUnionHeadRow(u: any): UnionHeadType {
  return {
    id: u.id,
    name: u.full_name || u.name || "—",
    phone: u.phone || "—",
    address: u.address || "—",
    park: u.park_name || u.park || "—",
    driversCount: u.drivers_count ?? 0,
    ...
    totalRevenue: formatNaira(u.total_revenue || u.card_balance),
...
const AccountsUnionHeadsTable: React.FC = () => {
  const [unionHeadsData, setUnionHeadsData] = useState<UnionHeadType[]>([]);
  const [search, setSearch] = useState("");
  const [currentPage, setCurrentPage] = useState(1);
  const [selectedRows, setSelectedRows] = useState<(number | string)[]>([]);
  const [openMenuId, setOpenMenuId] = useState<number | string | null>(null);
  const [selectedUnionHead, setSelectedUnionHead] = useState<UnionHeadType | null>(null);
  const [drawerOpen, setDrawerOpen] = useState(false);
  const [transactionsModalOpen, setTransactionsModalOpen] = useState(false);
  const [transactionsUser, setTransactionsUser] = useState<UnionHeadType | null>(null);
  ...
  useEffect(() => {
    api("/api/v1/users?role=UNION_HEAD")
      .then((rows) => setUnionHeadsData((rows || []).map(mapUnionHeadRow)))
      .catch(() => setUnionHeadsData([]));
  }, []);

  // Filter union heads based on search and filters
  const filteredUnionHeads = unionHeadsData.filter((unionHead) => {
    const matchesSearch = unionHead.name.toLowerCase().includes(search.toLowerCase()) ||
                         unionHead.phone.includes(search) ||
                         unionHead.park.toLowerCase().includes(search.toLowerCase());
    const matchesStatus = appliedFilters.status === "all" || unionHead.status === appliedFilters.status;
    const matchesPark = appliedFilters.park === "all" || unionHead.park === appliedFilters.park;
    
    return matchesSearch && matchesStatus && matchesPark;
  });

  const totalPages = Math.ceil(filteredUnionHeads.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const paginatedUnionHeads = filteredUnionHeads.slice(startIndex, startIndex + itemsPerPage);

  const handleViewProfile = (unionHead: UnionHeadType) => {
    setSelectedUnionHead(unionHead);
    setDrawerOpen(true);
    setOpenMenuId(null);
  };

  const handleTransactions = async (unionHead: UnionHeadType) => {
    setTransactionsUser(unionHead);
    try {
      const rows = await api(`/api/v1/payments?userId=${unionHead.id}`);
      setTransactions(
        (rows || []).map((p: any) => {
  ...
  const handleDownloadReport = () => {
    const reportData = filteredUnionHeads.map(unionHead => ({
      Name: unionHead.name,
      Phone: unionHead.phone,
      Address: unionHead.address,
      Park: unionHead.park,
      "Drivers Count": unionHead.driversCount,
      Status: unionHead.status,
      "Date Registered": unionHead.dateRegistered,
    }));
    exportToCSV(reportData, "union_heads_report");
  };

  // Get unique parks for filter
  const uniqueParks = Array.from(new Set(unionHeadsData.map(uh => uh.park)));
  ...
  const handleSelectAll = () => {
    if (selectedRows.length === paginatedUnionHeads.length) {
      setSelectedRows([]);
    } else {
      setSelectedRows(paginatedUnionHeads.map(unionHead => unionHead.id));
    }
  };

  const isAllSelected = paginatedUnionHeads.length > 0 && selectedRows.length === paginatedUnionHeads.length;
  const isIndeterminate = selectedRows.length > 0 && selectedRows.length < paginatedUnionHeads.length;
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <h3 style={{ margin: 0, fontSize: "18px", fontWeight: "600" }}>Filter Union Heads</h3>
          <button
        ...
        <div style={{ marginBottom: "1rem" }}>
          <label style={{ display: "block", marginBottom: "0.5rem", fontWeight: "500" }}>Park</label>
          <select
            value={draftFilters.park}
            onChange={(e) => setDraftFilters(prev => ({ ...prev, park: e.target.value }))}
            style={{
              width: "100%",
              padding: "8px 12px",
              border: "1px solid #ddd",
  ...
  return (
    <div style={{ background: "#fff", borderRadius: 16, padding: "2rem", boxShadow: "0 2px 8px rgba(0,0,0,0.03)" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          ...
          <div style={{ position: "relative" }}>
            <input
              type="text"
              placeholder="Search union heads..."
              value={search}
      ...
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        ...
        <tbody>
            {paginatedUnionHeads.map((unionHead) => (
              <tr key={unionHead.id} style={{ 
                borderBottom: "1px solid #f1f5f9",
                background: selectedRows.includes(unionHead.id) ? "#f0fdf4" : undefined 
              }}>
                <td style={{ padding: "1rem", fontSize: "14px", color: "#1f2937" }}>
                  <input
                    type="checkbox"
                    checked={selectedRows.includes(unionHead.id)}
                    onChange={() => handleSelectRow(unionHead.id)}
                    style={{ cursor: "pointer" }}
                  />
                </td>
                <td style={{ padding: "1rem", fontSize: "14px", color: "#1f2937" }}>
                ...
                <td style={{ padding: "1rem", fontSize: "14px", color: "#1f2937" }}>
                {unionHead.driversCount} drivers
              </td>
                <td style={{ padding: "1rem", textAlign: "center", position: "relative" }}>
                <button
                    onClick={() => setOpenMenuId(openMenuId === unionHead.id ? null : unionHead.id)}
                  style={{
                    background: "none",
                    border: "none",
                    cursor: "pointer",
                ...
                {openMenuId === unionHead.id && (
                    ...
                    <button
                        onClick={() => handleViewProfile(unionHead)}
                      style={{
                        width: "100%",
                    ...
                    <button
                        onClick={() => handleTransactions(unionHead)}
                      style={{
                        width: "100%",
                          padding: "0.75rem 1rem",
                        border: "none",
      ...
      {/* Union Head Details Drawer */}
      <UnionHeadDetailsDrawer
        isOpen={drawerOpen}
        onClose={() => setDrawerOpen(false)}
        unionHead={selectedUnionHead}
      />
      {/* Transactions Modal */}
      <TransactionsModal
...
export default AccountsUnionHeadsTable;