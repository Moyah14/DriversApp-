...
const Parks: React.FC<ParksProps> = ({ searchModalOpen, setSearchModalOpen, searchValue, setSearchValue }) => {
  ...
  const [parkForm, setParkForm] = useState({ name: "", address: "", area: "" });
  ...
  // Handlers
  const openDetails = async (park: any) => {
    setSelectedPark(park);
    setDetailsDrawerOpen(true);
    try {
      const detail = await api(`/api/v1/parks/${park.id}`);
      if (detail) setSelectedPark(mapParkRow(detail));
    } catch {
      // keep list row data
    }
  };
  ...
  const handleAddPark = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api("/api/v1/parks", {
        method: "POST",
        body: JSON.stringify({
          name: parkForm.name || parkForm.address,
          area: parkForm.area || (selectedArea === "All Areas" ? "Jos North" : selectedArea),
          address: parkForm.address,
          tenantId,
        }),
      });
      setAddModalOpen(false);
      setParkForm({ name: "", address: "", area: "" });
      loadParks();
    } catch (err: any) {
      setFormError(err.message || "Failed to add park");
    } finally {
      setFormSaving(false);
    }
  };
    ...
    <div style={{ display: "flex" }}>
      ...
      >
        ...
        <main style={{ padding: "2rem", background: "#f5f5f5", minHeight: "100vh" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                {filteredParks.map(park => (
                  <div
                    ...
                    onClick={() => openDetails(park)}
                  >
                    {/* Top Section with Park Name, Drivers Count, and Arrow */}
                    <div style={{ 
                      display: "flex", 
                      justifyContent: "space-between", 
                      alignItems: "center",
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                        <div style={{ fontWeight: 600, fontSize: 18, color: "#222" }}>{park.name}</div>
                        <span style={{
                        ...
                        }}>{park.driversCount} Drivers</span>
                      </div>
                      <div style={{
                        width: "24px",
                        height: "24px",
                    ...
                    <div style={{ 
                      ...
                      <span style={{ color: "#222", fontSize: 14, textAlign: "right", flex: 1, marginLeft: "16px" }}>{park.address}</span>
                    </div>
                    
                    {/* Bottom Section - Union Head */}
                    <div style={{ 
                      display: "flex", 
                      justifyContent: "space-between", 
                      alignItems: "center"
                      ...
                      <span style={{ color: "#222", fontSize: 14, textAlign: "right", flex: 1, marginLeft: "16px" }}>{park.unionHead}</span>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div style={{
                ...
                <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", gap: 16, marginBottom: 32 }}>
                  <div style={{ width: 80, height: 80, borderRadius: "50%", background: "#ecfdf5", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <img src="/addparks.png" alt="Add Park" style={{ width: "48px", height: "48px" }} />
                  </div>
                  <h3 style={{ margin: 0, marginBottom: 24, fontSize: 22, fontWeight: "bold" }}>Add Park</h3>
                </div>
                <form onSubmit={handleAddPark}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 42, marginBottom: 32 }}>
                    <div>
                      <label style={{ display: "block", marginBottom: 8, fontWeight: 500 }}>Park Name*</label>
                      <input
                        type="text"
                    ...
                    <div>
                      ...
                      <select
                        value={parkForm.area}
                        onChange={(e) => setParkForm((f) => ({ ...f, area: e.target.value }))}
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setParkForm((f) => ({ ...f, address: e.target.value }))}
                        style={{ width: "100%", padding: 12, border: "1px solid #ddd", borderRadius: 8, fontSize: 15 }}
                        required
                      />
                    </div>
                  ...
                  <div style={{ display: "flex", justifyContent: "flex-end", gap: 16 }}>
                    ...
                    <button type="submit" disabled={formSaving} style={{ background: "#16a34a", color: "#fff", border: "none", borderRadius: 8, padding: "12px 32px", fontWeight: 600, fontSize: 16, cursor: "pointer" }}>{formSaving ? "Saving..." : "Add Park"}</button>
                  </div>
                </form>
              </div>
            </div>
          ...
          {/* Park Details Drawer */}
          {detailsDrawerOpen && selectedPark && (
            <>
              ...
              <div style={{
                ...
                <button
                  ...
                  aria-label="Close"
                >×</button>
                <h2 style={{ marginTop: 0, marginBottom: 24, fontSize: 24, fontWeight: 700, color: "#111827" }}>Park Details</h2>
                ...
                <div style={{ fontWeight: 600, fontSize: 20, marginBottom: 32, color: "#111827" }}>{selectedPark.name}</div>
                
                {/* Park Information Section */}
                <div style={{ display: "flex", flexDirection: "column", gap: 24, marginBottom: 40 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span style={{ color: "#888", fontSize: 14, fontWeight: 500 }}>Address</span>
                    <span style={{ color: "#111827", fontSize: 14, textAlign: "right", flex: 1, marginLeft: "16px" }}>{selectedPark.address}</span>
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span style={{ color: "#888", fontSize: 14, fontWeight: 500 }}>Union Head</span>
                    <span style={{ color: "#111827", fontSize: 14, textAlign: "right", flex: 1, marginLeft: "16px" }}>{selectedPark.unionHead}</span>
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span style={{ color: "#888", fontSize: 14, fontWeight: 500 }}>Date Registered</span>
                    <span style={{ color: "#111827", fontSize: 14, textAlign: "right", flex: 1, marginLeft: "16px" }}>{selectedPark.dateRegistered}</span>
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span style={{ color: "#888", fontSize: 14, fontWeight: 500 }}>Registered By</span>
                    <span style={{ color: "#111827", fontSize: 14, textAlign: "right", flex: 1, marginLeft: "16px" }}>{selectedPark.registeredBy}</span>
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <div style={{ marginBottom: 32 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
                    <div style={{ fontWeight: 600, fontSize: 16, color: "#111827" }}>Drivers Registered ({selectedPark.driversCount})</div>
                    <button style={{ 
                      background: "none", 
                      border: "none", 
                      color: "#16a34a", 
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 32 }}>
                    {selectedPark.drivers?.slice(0, 6).map((driver: any, idx: number) => (
                      <div key={idx} style={{ 
                        display: "flex", 
                        alignItems: "center", 
                        gap: 16, 
